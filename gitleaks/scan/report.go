...
func WriteReport(report Report, opts options.Options, cfg config.Config) error {
	...
	if opts.Report != "" {
		...
		case "csv":
			...
			err = w.Write([]string{"repo", "line", "commit", "offender", "leakURL", "rule", "tags", "commitMsg", "author", "email", "file", "date", "branchName"})
			if err != nil {
				return err
			}
			for _, leak := range report.Leaks {
				err := w.Write([]string{leak.Repo, leak.Line, leak.Commit, leak.Offender, leak.LeakURL, leak.Rule, leak.Tags, leak.Message, leak.Author, leak.Email, leak.File, leak.Date.Format(time.RFC3339), leak.BranchName})
				if err != nil {
					return err
				}
			}
