	...
	err = fIter.ForEach(func(f *object.File) error {
		...
		for _, rule := range fs.cfg.Rules {
			...
			if rule.HasFileOrPathLeakOnly(f.Name) {
				command := "cd " + fs.opts.ClonePath + "; git name-rev " + fs.commit.Hash.String() + " --name-only"
				branchName, err := exec.Command("/bin/sh", "-c", command).Output()
				if err != nil {
					log.Info("Unable to get the branch name ", err)
				}
				...
				leak.BranchName = strings.Trim(string(branchName), "\n")
